(function () {
  const WA = "639983742271";
  const LEAD_MIN = 45;
  const SLOTS = ["16:00", "16:30", "17:00", "17:30", "18:00", "18:30", "19:00", "19:30", "20:00", "20:30", "21:00", "21:30", "22:00", "22:30", "23:00", "23:30"];
  ...
  function fillDays() {
    ...
    for (let i = 0; i < 10; i += 1) {
      ...
      if (next.getUTCDay() === 1) continue; // closed Mondays
  ...
  function fillSlots() {
    ...
    if (!slotEl.options.length) {
      const opt = document.createElement("option");
      opt.value = "";
      opt.textContent = "Too late to pre-order — just walk in, open till 5 a.m.";
      slotEl.appendChild(opt);
    } else if (keep) {
      slotEl.value = keep;
    }
